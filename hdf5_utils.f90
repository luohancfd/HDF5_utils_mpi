...
contains
  ...
  !  \brief writes a scalar to an hdf5 file
  subroutine hdf_write_dataset_character_0(loc_id, dset_name, array, chunks, filter)
    ...
    ! write dataset
    call h5dwrite_f(dset_id, dtype_id, array, dims, hdferror)
    !write(*,'(A20,I0)') "h5dwrite: ", hdferror
  ...
  !  \brief writes an array of fixed length characters to an hdf5 file
  subroutine hdf_write_dataset_character_1(loc_id, dset_name, array, chunks, filter)
  ...
  !  \brief writes an array of fixed length characters to an hdf5 file
  subroutine hdf_write_dataset_character_2(loc_id, dset_name, array, chunks, filter)
  ...
  subroutine hdf_read_dataset_character_0(loc_id, dset_name, array)
    ...
    ! write dataset
    call h5dread_f(dset_id, dtype_id, array, dims, hdferror)
  ...
  subroutine hdf_read_dataset_character_1(loc_id, dset_name, array)
    ...
    integer :: rank
    integer(SIZE_T) :: dims_in(1), dims(1), max_dims(1), length, ii
    integer(HID_T) :: dset_id, dspace_id, dtype_id
    ...
    if (length == len(array(1))) then
      ...
      call h5dread_f(dset_id, dtype_id, buffer, dims, hdferror)
      do ii = 1, dims(1)
        array(ii) = buffer(ii)
      end do
      deallocate(buffer)
